...
4. Add or update tests. KitoPaywall is iOS-only (StoreKit 2 + SwiftUI), so
   run the suite on a simulator:
   `xcodebuild test -scheme KitoPaywall -destination "platform=iOS Simulator,name=iPhone 17 Pro"`.
...
- [ ] Builds cleanly for the iOS simulator, with no warnings
- [ ] Tests pass, and new behavior has new tests (pricing maths and
      entitlement rules are pure functions, so they're easy to cover)
- [ ] Purchase-flow changes were tried against the bundled `.storekit`
      configuration, not only against preview plans
